...
public class DownloadTasksManagerCountDownLatch {
    ...
    public DownloadTasksManagerCountDownLatch(List<String> fileNames, File folder, GUI parentGUI,
	...
	public void iniciarDownload() { 
		for (int i = 0; i < fileNames.size();i++) {
			...
			int totalBlocks = totalBlocksList.get(i);
            new Thread(() -> { //Cria uma thread para cada ficheiro para esperar a chegada de todos os blocos desse ficheiro
                try {
                    CountDownLatch fileLatch = new CountDownLatch(totalBlocks);
                    ...
                    while (fileLatch.getCount() > 0) {
                        ...
                        if (mensagem.getFilename().equals(fileName)) {
                            ...
                            fileDataMap.get(fileName)[blockIndex] = mensagem.getData();
                            fileLatch.countDown();
                        } else {
                            // Reinsere a mensagem na fila para outras threads processarem
	...
	private void criarBlockRequests(Map<String, List<Integer>> portsWithFile) {
	    ...
	    // Inicializar filas de blocos por porto
	    ...
	    try {
	        ...
	        // Criar threads para cado porto (apenas uma thread por porto)
    ...
    private void salvarArquivo(String fileName, byte[][] fileData) throws IOException {
        ...
        currentNode.removeDownloadManagerLatch(fileName); // Remover o manager
    }
